...
begin
  if p_status not in ('draft', 'confirmé', 'livré', 'annulé', 'retourné') then
    raise exception 'Statut de commande invalide: %', p_status;
  end if;
  ...
  if v_current_status in ('livré', 'annulé') then
    raise exception 'Le statut % est définitif', v_current_status;
  end if;

  if p_status in ('annulé', 'retourné') then
    for v_item in
  ...
  elsif v_current_status = 'retourné' then
    for v_item in
      select product_id, sum(quantity)::integer as quantity
      from public.order_items
      where order_id = p_order_id
    ...
    loop
      ...
      if not found then
        raise exception 'Stock insuffisant pour réactiver la commande';
      end if;
