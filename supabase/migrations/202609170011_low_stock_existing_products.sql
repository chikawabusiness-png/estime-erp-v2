...
select
  ...
  jsonb_build_object(
    ...
    'new', jsonb_build_object(
      ...
      'sku', p.sku,
      'stock', p.stock,
      'minimum_stock', public.get_product_min_stock(p),
      'threshold_mode', p.stock_threshold_mode,
      'fixed_min_stock', p.fixed_min_stock
    )
  )
from public.products p
where public.get_product_min_stock(p) is not null
  and p.stock <= public.get_product_min_stock(p)
  and not exists (
    select 1
    from public.telegram_alert_queue q
    where q.event_type = 'products.low_stock'
      and q.payload->'new'->>'id' = p.id::text
