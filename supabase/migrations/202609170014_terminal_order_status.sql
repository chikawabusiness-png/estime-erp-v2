...
begin
  ...
  if v_current_status in ('livré', 'annulé') then
    raise exception 'Le statut % est définitif et ne peut plus être modifié', v_current_status;
  end if;
    ...
    loop
      ...
      values (v_item.product_id, 'entrée', v_item.quantity,
        case when p_status = 'retourné'
          then 'Retour commande #' else 'Annulation commande #' end
        || left(p_order_id::text, 8), auth.uid());
    end loop;
  elsif v_current_status = 'retourné'
